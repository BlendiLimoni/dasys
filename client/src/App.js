...
function App() {
    ...
    newSocket.on('draw-element', (element) => {
      ...
      if (element.type === GLOBAL_CLEAR_COMMAND_TYPE) {
        ...
        if (contextRef.current && canvasRef.current) {
          ...
          console.error('Cannot execute global clear: canvas or context not ready.');
        }
      ...
      const elementExists = elementsRef.current.some(el => el.id === element.id);
  ...
  const draw = (e) => {
    ...
    switch (currentElement.type) {
      case 'pencil':
        ...
        const lastPoint = currentElement.points[currentElement.points.length - 2];
        ctx.beginPath();
    ...
    switch (element.type) {
      ...
      case 'text':
        ...
        return x >= element.x && x <= element.x + textWidth &&
               y >= element.y - parseInt(element.font) && y <= element.y;
      default:
        return false;
    }
  };
  ...
  useEffect(() => {
    ...
    if (storedElements || backupImage) {
      ...
      if (storedElements && backupImage && confirm(
        `Unsaved drawing data found from ${new Date(backupTime).toLocaleString()}. Would you like to restore it?`
      )) {
        try {
          ...
          if (Array.isArray(parsedElements) && parsedElements.length > 0) {
            console.log(`Restored ${parsedElements.length} elements from local storage`);
  ...
  const diagnoseConnection = () => {
    ...
    socket.emit('ping', {}, () => {
      ...
      if (pingTime > 500) {
        setConnectionError(`High latency detected (${pingTime}ms). This may cause synchronization delays.`);
      } else {
  ...
  return (
    <div className="container">
      ...
      />
      <NetworkInfo connected={connected} clientId={clientId} />
